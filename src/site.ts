...
export type SiteConfig = Pick<
  FullConfig,
  "pages" | "exclude" | "sourcesDir" | "pageExtension"
>;
export type PageStructure = SiteConfig["pages"];
export type PageAssigner = (
  item: DocItem,
  file: SourceUnit,
  config: SiteConfig
) => string | undefined;
...
export function buildSite(
  ...
  const assign =
    typeof siteConfig.pages === "string"
      ? pageAssigner[siteConfig.pages]
      : siteConfig.pages;
  ...
  for (let { input, output } of builds) {
    ...
    for (const { ast } of Object.values(output.sources)) {
      ...
      for (const topLevelItem of file.nodes) {
        ...
        const page = assignIfIncludedSource(
          assign,
          topLevelItem,
          file,
          siteConfig
        ...
        for (const item of topLevelItem.nodes) {
          ...
          if (isNewFile && page !== undefined)
            items.push(item as DocItemWithContext);
          const contract =
            topLevelItem.nodeType === "ContractDefinition"
              ? topLevelItem
              : undefined;
  ...
  return {
    items,
    pages: Object.entries(pages).map(([id, pageItems]) => ({
      id,
      items: pageItems,
    })),
  };
...
function defineContext(
  ...
  return Object.assign(item, {
    [DOC_ITEM_CONTEXT]: {
      ...
      contract,
      page,
      item: item as DocItemWithContext,
    },
  });
}
...
function defineProperties(item: DocItemWithContext, properties: Properties) {
  for (const [prop, fn] of Object.entries(properties)) {
    ...
    defineGetterMemoized(item as any, prop, () =>
      fn(item.__item_context, original)
    );
  }
}
